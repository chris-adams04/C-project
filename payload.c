// payload.c
// Compile:
//   cl payload.c /link user32.lib advapi32.lib wininet.lib
// or:
//   gcc payload.c -o flashplayer_install.exe -luser32 -ladvapi32 -lwininet
...
/*void PrintLastError(const char *msg) {
    ...
    LPSTR errorMsg = NULL;
    FormatMessageA(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM,
                   NULL, dwError, 0, (LPSTR)&errorMsg, 0, NULL);
    printf("%s: %s\n", msg, errorMsg);
    LocalFree(errorMsg);
}
...
void LaunchCmdThenPaint() {
    ...
    // Launch CMD and keep it open with "start" command
    if (!CreateProcessA("C:\\Windows\\System32\\cmd.exe", 
                        "/K start",  // "/K" keeps the CMD window open
                        NULL, NULL, FALSE, 0, NULL, NULL, &si, &piCmd)) {
        PrintLastError("CreateProcess CMD");
        return;
    }

    // Wait a moment to ensure CMD is open before launching Paint
    Sleep(500);

    // Now launch Paint from the already opened CMD
    if (!CreateProcessA("C:\\Windows\\System32\\mspaint.exe", 
                        NULL, NULL, NULL, FALSE, 0, NULL, NULL, &si, &piPaint)) {
        PrintLastError("CreateProcess Paint");
        return;
    }

    printf("[Process] CMD and Paint launched.\n");

    // Wait for CMD and Paint to finish
    WaitForSingleObject(piCmd.hProcess, INFINITE);
    WaitForSingleObject(piPaint.hProcess, INFINITE);
}
*/
